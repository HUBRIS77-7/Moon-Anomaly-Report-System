...
func _build_active_layer() -> void:
	...
	const COL_L := 374     # left column width
	const COL_R := 254     # right column width (W - PAD*3 - COL_L)
	...
	_transcription_rtl.scroll_active = true
	...
	_transcription_rtl.add_theme_color_override("default_color", C_TEXT)
	...
	_extra_rtl.add_theme_color_override("default_color", C_DIM)
	_extra_rtl.add_theme_font_size_override("normal_font_size", 11)
	extra_panel.add_child(_extra_rtl)

	# ── RIGHT COLUMN ──────────────────────────────────────────────────────────
	const TASKS_H  := 130
	const ANOM_H   := 600 - LEFT_Y - TASKS_H - 48 - PAD * 4
	...
	search.add_theme_font_size_override("font_size", 10)
	...
	_submit_btn.position = Vector2(COL_R_X, 600 - PAD - 40)
