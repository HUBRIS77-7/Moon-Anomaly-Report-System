...
# Map each static body to its viewport and size
var _screen_map: Dictionary = {}

func _ready() -> void:
	var file = FileAccess.open("res://tree_dump.txt", FileAccess.WRITE)
	_print_tree_to_file(get_node("/root"), 0, file)
	file.close()
	print("Tree dumped")
		
	print("PanelMesh: ", screen_panel_mesh)
	print("InfoMesh: ", screen_info_mesh)
	...
	# Build screen map after meshes exist
	# Key = StaticBody3D RID, Value = {viewport, size}
...
func _print_tree_to_file(node: Node, depth: int, file: FileAccess) -> void:
	file.store_line("  ".repeat(depth) + node.name + " (" + node.get_class() + ")")
	for child in node.get_children():
		_print_tree_to_file(child, depth + 1, file)
...
func _forward_event(event: InputEvent, viewport: SubViewport, pos: Vector2) -> void:
	...
	elif new_event is InputEventMouseMotion:
		new_event.position = pos
		new_event.global_position = pos
		viewport.push_input(new_event)
